...
const Warehouse = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      instance.get("/robots/").then((requsets) => {
        ...
        requsets.data.map(function (robot) {
          ...
          area = zone4.find((x) => x.num == robot.area);
          if (area) {
            if (!area.hasOwnProperty("robots")) {
              area.robots = [];
            }
              ...
              <div key={item.id}>
                <div className={styles.area} key={item.id}>
                  ...
                  <div className={styles.robots}>
                    {item.robots && item.robots.map(function (robot) {
                      return(<p key={robot.id} className={styles.robot}>•{robot}</p>)
                    })}
                  </div>
